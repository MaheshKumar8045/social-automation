...
def test_refresh_package_keeps_canonical_identity_when_optional_visual_tables_are_missing(tmp_path):
    import json
    import sqlite3
    from core.refresh_media_prompts import refresh_package
    ...
    plan["characters"] = [{
        ...
        "scene_mentions": [],
        "visual_profile": {"identity_anchor": "vib-shiva", "source_facts": [], "inferred_facts": []},
    }]
    ...
    package = {
        ...
        "source_database": str(db),
        "scene_count": 1,
        "scenes": [{"scene_id": 1, "story_id": 1, "scene_order": 1, "title": "I. The end — Scene 1", "plan": plan}],
    }
    source = tmp_path / "all_prompts.json"
    source.write_text(json.dumps(package), encoding="utf-8")
    output = tmp_path / "refresh"
...
def test_refresh_package_prefers_local_structure_db_and_preserves_canonical_identity_on_collision(tmp_path):
    import json
    import sqlite3
    from core.refresh_media_prompts import refresh_package

    document_dir = tmp_path / "data"
    prompts = document_dir / "Asura - Tale Of The Vanquished_structure_prompts"
    prompts.mkdir(parents=True)
    db = document_dir / "Asura - Tale Of The Vanquished_structure.db"
    ...
    package = {
        "schema_version": 2,
        "document_id": 1,
        # Deliberately point at a stale/missing path. The local structure DB
        # beside the *_structure_prompts directory must win.
        "source_database": str(tmp_path / "stale_structure.db"),
        "scene_count": 1,
        "scenes": [{
            "scene_id": 1,
            "story_id": 1,
            "scene_order": 1,
            "title": "I. The end — Scene 1",
            "plan": plan,
        }],
    }
