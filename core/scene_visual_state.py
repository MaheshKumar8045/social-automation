...
def build_scene(db: str|Path, doc:int, scene:int)->dict:
 with sqlite3.connect(db) as con:
  ...
  rows=con.execute('''SELECT ccp.id profile_id,ccp.canonical_character_id,ccp.canonical_name,ccp.status,ccp.confidence,
      ...
      WHERE ccp.document_id=? AND (cvf.scene_id=? OR cvf.scene_id IS NULL)
  ...
  for r in rows:
   ...
   # Scene-scoped facts are preferred. Stable facts with no scene are inherited.
...
def main():
 p=argparse.ArgumentParser(description='Return source-grounded scene-effective visual state'); p.add_argument('database'); p.add_argument('document_id',type=int); p.add_argument('scene_id',type=int); a=p.parse_args(); print(json.dumps(build_scene(a.database,a.document_id,a.scene_id),ensure_ascii=False,indent=2))
