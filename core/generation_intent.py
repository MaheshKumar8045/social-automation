...
_CHARACTER_PHYSICAL_RE = re.compile(r"\b(?:approach\w*|arriv\w*|attack\w*|capture\w*|climb\w*|come|cross\w*|cry\w*|die\w*|enter\w*|fall\w*|flee\w*|follow\w*|fight\w*|fought|grab\w*|hold\w*|kill\w*|look\w*|move\w*|open\w*|reach\w*|return\w*|run\w*|save\w*|sit\w*|stand\w*|take\w*|turn\w*|walk\w*|watch\w*|travel\w*|strike\w*|kneel\w*|rise\w*|speak\w*|was\s+there|were\s+there|is\s+there|are\s+there|was\s+present|were\s+present|is\s+present|are\s+present|stood|sat|lay|remained|waited|rested|entered|arrived|appeared|left|returned|looked|watched|faced|knelt|rose|walked|ran|fled|followed|held|carried|spoke|sang|wept|cried)\b", re.I)
    ...
    for character in characters:
        ...
        if has_authoritative_presence:
            if source_presence.get("physical_presence") is True:
                ...
                elif matching_events or re.search(rf"\b{name_pattern}\b", scene_text, re.I):
