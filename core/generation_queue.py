...
def enqueue(database_path: str | Path, document_id: int, scene_id: int, job_type: str, scheduled_at: str | None = None) -> dict[str, Any]:
    ...
    with sqlite3.connect(database_path) as con:
        ...
        con.commit()
        return {"queue_id": cur.lastrowid, "document_id": document_id, "scene_id": scene_id, "job_type": job_type, "scheduled_at": when, "status": "queued"}
...
def run_due(database_path: str | Path, limit: int = 1) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if provider is None:
            with sqlite3.connect(database_path) as con:
                con.execute("UPDATE generation_queue SET last_error=?,updated_at=? WHERE id=?", ("no eligible provider", _now(), row["id"]))
                con.commit()
        ...
        try:
            ...
            with sqlite3.connect(database_path) as con:
                ...
                con.commit()
            results.append({"queue_id": row["id"], "status": "completed", "job": result})
        except Exception as exc:
            with sqlite3.connect(database_path) as con:
                con.execute("UPDATE generation_queue SET attempts=attempts+1,last_error=?,updated_at=? WHERE id=?", (str(exc), _now(), row["id"]))
                con.commit()
