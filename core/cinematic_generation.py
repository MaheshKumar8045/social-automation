...
def _clean_dialogue_candidate(sentence: str, scene: dict[str, Any] | None = None) -> str:
    ...
    for index, word in enumerate(words):
        ...
        if prefix and prefix[0].lower() not in {"i", "we", "my", "our", "you", "your"} and all(_NAME_TOKEN_RE.match(item) for item in prefix):
...
def _visual_moments(scene_text: str, events: list[dict[str, Any]], characters: list[dict[str, Any]]) -> list[str]:
    ...
    for event in events:
        ...
        if not text or text not in scene_text:
            continue
        score = 50 + (35 if _ACTION_RE.search(text) else 0) + (15 if _DESTRUCTION_RE.search(text) else 0) + (10 if _COMBAT_RE.search(text) else 0) + sum(10 for name in names if name and name in text.casefold())
...
def _camera_for(role: str, signal: str, has_visible: bool) -> dict[str, str]:
    profiles = {
        "establish": ("wide environmental establishing frame", "slow lateral drift", "24–28mm equivalent wide perspective"),
        "action": ("medium-wide action frame with clear spatial separation", "restrained tracking matched to the source action", "28–35mm equivalent natural perspective"),
        "consequence": ("wide-to-medium consequence frame emphasizing source-established aftermath", "slow push-in toward the documented consequence", "35mm equivalent natural perspective"),
        "detail": ("purposeful environmental or source-object detail", "slow micro-dolly or static hold", "50–85mm equivalent detail perspective"),
        "reaction": (("medium reaction frame" if has_visible else "environmental reaction frame"), "subtle push-in or static hold", "50mm equivalent natural perspective"),
        "movement": ("tracking composition preserving travel geography", "lateral or rear tracking move", "28–35mm equivalent wide-natural perspective"),
        "destination": ("forward-looking destination composition grounded in the source setting", "measured reveal or gentle forward move", "35mm equivalent natural perspective"),
        "develop": ("balanced medium-wide development composition", "slow arc or restrained dolly", "35–50mm equivalent natural perspective"),
        "close": ("balanced closing composition", "slow pull-back or gentle rise", "35–50mm equivalent natural perspective"),
    }
    framing, movement, lens = profiles.get(role, profiles["develop"])
