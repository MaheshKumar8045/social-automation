...
ROLE_TOKENS={'river','rivers','mount','mountain','mountains','lake','ocean','sea','island','islands','colony','republic','government','commission','observatory','institution','post','world','africa','hope','zambesi','cape','port','town','city','village','forest','valley','country','countrymen','empire','kingdom','company','society','school','university','museum','academy','station','road','roads','falls','fall','gulfs','gulf','desert','coast','shore','bay','peninsula','expedition','party','tribe','people','new','zealand','atlantic','balearic','central','subterranean'}
def norm(name:str)->str:
 s=re.sub(r'\s+',' ',name.replace('‐','-').replace('‑','-').replace('‒','-').replace('–','-').replace('—','-')).strip(' ,.;:\"\''); s=re.sub(r'\s+([,.;:])',r'\1',s); return s[:-1] if s.endswith('-') and len(s)>3 else s
def gate(name:str,entity_type:str,mentions:list[sqlite3.Row])->tuple[str,float,list[str]]:
 ...
 words=n.replace('-',' ').split(); title=bool(PERSON_TITLE.match(n)); bare=[w.strip('.') for w in words if w.lower() not in {'mr','mrs','ms','miss','dr','prof','professor','capt','captain','sir','lady','lord','rev','reverend','colonel','major','lieutenant','herr','monsieur','madame'}]
 ...
 score=max(0,min(1,score));decision='validated' if score>=.75 else 'probable' if score>=.48 else 'review';return decision,score,reasons
def build(db:str|Path,document_id:int)->dict[str,int]:
 with sqlite3.connect(db) as con:
  con.row_factory=sqlite3.Row;con.execute('PRAGMA foreign_keys=ON');con.executescript(SCHEMA);con.execute('DELETE FROM character_candidate_gate WHERE document_id=?',(document_id,)); entities=con.execute('SELECT id,entity_type,canonical_name FROM entities WHERE document_id=? ORDER BY id',(document_id,)).fetchall()
  for e in entities:
   mentions=con.execute('SELECT scene_id,context FROM entity_mentions WHERE document_id=? AND entity_id=? ORDER BY page_start,id',(document_id,e['id'])).fetchall();d,s,r=gate(e['canonical_name'],e['entity_type'],mentions);con.execute('INSERT INTO character_candidate_gate(document_id,entity_id,decision,normalized_name,score,reasons_json) VALUES(?,?,?,?,?,?)',(document_id,e['id'],d,norm(e['canonical_name']),s,json.dumps(r)))
...
def main()->None:
 p=argparse.ArgumentParser(description='Conservative character candidate gate');p.add_argument('database',type=str);p.add_argument('document_id',type=int);a=p.parse_args();counts=build(a.database,a.document_id);print('=== CHARACTER CANDIDATE GATE ===');[print(f'{k}: {counts.get(k,0)}') for k in ('validated','probable','review','non_character')]
