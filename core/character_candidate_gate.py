...
NON_PERSON = set('african english englishman european french icelandic icelanders russians danish makololos makololo bochjesmen bochjesmen queen earth orange reykjawik sneffels'.split())
...
def gate(name: str, entity_type: str, mentions: list[sqlite3.Row]) -> tuple[str, float, list[str]]:
    ...
    if speech_hits >= 1: score += 0.10; reasons.append('speech_context')
    ...
    # A single ordinary word can look like a person because it is capitalized
    # at sentence boundaries. Require an independent character cue before it
    # can reach validated/probable status.
    if len(bare_words) == 1 and not title and speech_hits == 0 and action_hits == 0:
        score = min(score, 0.44)
        reasons.append('single_word_without_character_context')
...
def build(db: str | Path, document_id: int) -> dict[str, int]:
    with sqlite3.connect(db) as con:
        ...
        for e in entities:
            ...
            con.execute('INSERT INTO character_candidate_gate(document_id,entity_id,decision,normalized_name,score,reasons_json) VALUES(?,?,?,?,?,?)', (document_id,e['entity_id'] if 'entity_id' in e.keys() else e['id'],decision,norm(e['canonical_name']),score,json.dumps(reasons)))
        con.commit()
