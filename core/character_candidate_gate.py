...
GEOGRAPHIC_OR_INSTITUTIONAL = re.compile(r'^(?:lake|mount|cape|port|river|ocean|sea|island|islands|falls|observatory|institution|government|commission|colony|africa|world|zambesi|gretchen|hope|new york|central sea|subterranean ocean)(?:\s|$)', re.I)
...
def gate(name: str, entity_type: str, mentions: list[sqlite3.Row]) -> tuple[str, float, list[str]]:
    ...
    if GEOGRAPHIC_OR_INSTITUTIONAL.match(n): return 'non_character', 1.0, ['geographic_or_institutional_name']
    ...
    speech_hits = sum(1 for x in contexts if SPEECH_CUE.search(x)); action_hits = sum(1 for x in contexts if ACTION_CUE.search(x))
    direct_hits = 0
    if n:
        direct = re.compile(DIRECT_PERSON_CUE.pattern.format(name=re.escape(n)), re.I)
    ...
    if len(bare_words) == 1 and not title and direct_hits == 0: score = min(score, 0.44); reasons.append('single_word_without_direct_person_reference')
    score = max(0.0, min(1.0, score)); decision = 'validated' if score >= 0.75 else ('probable' if score >= 0.48 else 'review')
