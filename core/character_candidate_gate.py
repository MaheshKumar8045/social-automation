...
OCR_FRAGMENT=re.compile(r'(?:^|\s)[A-Z][A-Za-z]{2,}-$')
...
def gate(name:str,entity_type:str,mentions:list[sqlite3.Row])->tuple[str,float,list[str]]:
 ...
 raw_name=name.strip()
 if raw_name.endswith(('-', '‐','‑','‒','–','—')): return 'review',0.4,['line_break_fragment']
 ...
 # Generic non-person morphology. A title overrides this check because titles
 # provide stronger evidence than the shared words in a place/institution name.
 ...
 direct_pat=re.compile(DIRECT_PERSON_CUE.pattern.format(name=re.escape(n)),re.I)
 direct=sum(1 for x in contexts if direct_pat.search(x))
 score=.25
