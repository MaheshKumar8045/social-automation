...
class VisualKnowledgeBible:
      ...
      with sqlite3.connect(self.database_path) as con:
       ...
       for cc in canon:
        ...
        for m in members:
         ...
         for x in mentions:
          # Match against the exact source alias present in the mention context.
          self._extract(con,pid,str(m['variant_name'] or cc['canonical_name']),int(x['scene_id']),int(x['page_start']),int(x['page_end']),str(x['context'] or ''),float(x['confidence'] or .5))
    ...
    def _extract(self,con,pid,name,sid,ps,pe,text,bc):
      nr=re.escape(name.strip());local=re.compile(rf'(?is)(?:\b(?:the|a|an)\s+)?{nr}[^.!?;:]{0,120}?(?:\b(?:was|were|is|had|has|looked|appeared|seemed|stood|wore|wearing|dressed|clad|possessed|with)\b)[^.!?;:]{0,100}|(?:\b(?:his|her|their)\b)[^.!?;:]{0,100}?{nr}[^.!?;:]{0,80}')
      ...
      for attr,patterns in self.CHARACTER_PATTERNS.items():
       for pat in patterns:
        for m in pat.finditer(text):
         ...
         self._add(con,pid,'appearance' if attr not in {'clothing','expression','mannerism'} else attr,attr,val,'scene',None,sid,ps,pe,self._evidence(text,m.start(),m.end()),min(.98,max(.35,bc)),'source_pattern_local')
    ...
    def _objects(self,con,d,scenes):
      ids={}
      for s in scenes:
       words=set(re.findall(r"\b[A-Za-z][A-Za-z'-]+\b",str(s['text'] or '').lower()))
       for term in sorted(words & self.OBJECT_TERMS):
        ...
        ids[term.title()]=oid;ev=self._term_evidence(str(s['text'] or ''),term);con.execute('INSERT OR IGNORE INTO visual_object_mentions(document_id,object_id,scene_id,page_start,page_end,evidence,confidence) VALUES(?,?,?,?,?,?,?)',(d,oid,int(s['id']),int(s['page_start']),int(s['page_end']),ev,.55))
      return ids
    def _scene_context(self,con,d,scenes,entity_profiles,objects):
      for s in scenes:
       chars=[]
       for r in con.execute('SELECT DISTINCT entity_id FROM entity_mentions WHERE document_id=? AND scene_id=?',(d,int(s['id']))).fetchall():
        pid=entity_profiles.get(int(r[0]));
        if pid is not None:chars.append(pid)
       con.execute('INSERT INTO scene_visual_context(document_id,scene_id,character_profile_ids,environment_profile_ids,object_mentions,continuity_json,evidence_json) VALUES(?,?,?,?,?,?,?)',(d,int(s['id']),json.dumps(sorted(set(chars))),json.dumps([]),json.dumps(list(objects.values())),json.dumps({'source_grounded':True,'unknowns_must_remain_unknown':True},sort_keys=True),'[]'))
    @staticmethod
...
def main():
 p=__import__('argparse').ArgumentParser();p.add_argument('database');p.add_argument('document_id',type=int);a=p.parse_args();r=build_visual_knowledge_bible(a.database,a.document_id);print('=== VISUAL KNOWLEDGE BIBLE ===');[print(f'{k}: {v}') for k,v in r.items()]
