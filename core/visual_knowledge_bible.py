...
# Canonical-character integration guard: character visual profiles are created
# only for records admitted by character_canonicalizer. Raw entities remain the
# source/provenance layer and must not repopulate the visual bible.
...
class VisualKnowledgeBible:
    ...
    ENV_PATTERNS={'weather':re.compile(r'\b((?:cold|hot|warm|freezing|icy|snowy|stormy|windy|foggy|misty|rainy|raining|sunny|dark|cloudy|clear|humid|dry)\s+(?:weather|air|wind|sky|day|night))\b',re.I),'terrain':re.compile(r'\b((?:steep|rocky|rugged|rough|smooth|flat|narrow|wide|deep|vast|dark|icy|snow-covered|snowy|volcanic|sandy|muddy|forested|barren)\s+(?:mountain|mountains|valley|plain|plains|road|shore|bank|banks|cave|cavern|tunnel|gallery|shaft|coast|island|islands|ground|terrain))\b',re.I),'light':re.compile(r'\b((?:bright|dim|dark|faint|brilliant|dazzling|sunlit|moonlit|shadowy|gloomy|red|blue|golden)\s+(?:light|glow|illumination|daylight|sunlight|moonlight))\b',re.I),'atmosphere':re.compile(r'\b((?:silent|quiet|noisy|oppressive|eerie|mysterious|terrible|terrific|beautiful|magnificent|desolate|lonely|cheerful|pleasant|stifling|suffocating|humid|damp|dry)\s+(?:atmosphere|air|place|scene|silence|stillness))\b',re.I),'material':re.compile(r'\b((?:rock|stone|ice|snow|sand|water|lava|basalt|granite|clay|wooden|wood|metal|iron|steel|glass|leather|brick|marble)\s+(?:wall|walls|floor|floors|ceiling|rock|rocks|ground|surface|door|doors|bridge|structure|material))\b',re.I)}
    ...
    def build(self,document_id):
        with sqlite3.connect(self.database_path) as con:
            ...
            # Only canonical character records may create character visual profiles.
            canon=con.execute('''SELECT cc.id canonical_id,cc.identity_group_id,cc.canonical_name,cc.confidence,cc.status,ig.canonical_entity_id
                FROM canonical_characters cc JOIN character_identity_groups ig ON ig.id=cc.identity_group_id
                WHERE cc.document_id=? AND cc.status IN ('confirmed','likely','singleton') ORDER BY cc.id''',(document_id,)).fetchall()
            entity_profiles={}
            for cc in canon:
                eid=int(cc['canonical_entity_id']) if cc['canonical_entity_id'] is not None else None
                pid=self._profile(con,document_id,eid,'character',cc['canonical_name'],float(cc['confidence'] or 0));
                if eid is not None: entity_profiles[eid]=pid
                ...
                for m in members:
                    entity_profiles[int(m['entity_id'])]=pid
                    mentions=con.execute('SELECT scene_id,page_start,page_end,context,confidence FROM entity_mentions WHERE document_id=? AND entity_id=? ORDER BY page_start,id',(document_id,int(m['entity_id']))).fetchall()
                    for x in mentions:self._extract_character_facts(con,pid,cc['canonical_name'],int(x['scene_id']),int(x['page_start']),int(x['page_end']),str(x['context'] or ''),float(x['confidence'] or .5))
    ...
    def _clear_document(self,con,d):
    ...
    @staticmethod
    def _profile(con,d,e,t,n,c):
        con.execute('INSERT INTO visual_profiles(document_id,entity_id,profile_type,canonical_name,confidence) VALUES(?,?,?,?,?)',(d,e,t,n,c));return con.execute('SELECT last_insert_rowid()').fetchone()[0]
    def _extract_character_facts(self,con,pid,name,sid,ps,pe,text,bc):
        nr=re.escape(name.strip());local=re.compile(rf'(?is)(?:\b(?:the|a|an)\s+)?{nr}[^.!?;:]{0,120}?(?:\b(?:was|were|is|had|has|looked|appeared|seemed|stood|wore|wearing|dressed|clad|possessed|with|of)\b)[^.!?;:]{0,100}|(?:\b(?:his|her|their)\b)[^.!?;:]{0,100}?{nr}[^.!?;:]{0,80}')
        spans=[m.span() for m in local.finditer(text)]
        if not spans:return
        for attr,patterns in self.CHARACTER_PATTERNS.items():
            for pat in patterns:
                for m in pat.finditer(text):
                    if not any(a<=m.start()<=b or a<=m.end()<=b for a,b in spans):continue
                    val=self._clean(m.group(1) if m.lastindex else m.group(0))
                    if not val:continue
                    if attr=='build' and not re.search(rf'(?is)(?:{nr}|\b(?:his|her|their)\b)[^.!?;:]{0,80}\b(?:was|were|is|looked|appeared|stood|seemed)\b[^.!?;:]{0,60}\b(?:tall|short|large|small|slender|thin|lean|slight|stout|stocky|broad|muscular|powerful|robust|strong|weak|lanky|massive|heavy|delicate)\b|\b(?:tall|short|large|small|slender|thin|lean|slight|stout|stocky|broad|muscular|powerful|robust|strong|weak|lanky|massive|heavy|delicate)\b[^.!?;:]{0,60}\b(?:man|woman|person|figure|fellow|hunter|astronomer|colonel|professor|doctor)\b',text):continue
                    if attr=='expression' and not re.search(rf'(?is)(?:{nr}|\b(?:his|her|their)\b)[^.!?;:]{0,70}\b(?:face|features|countenance|expression|look|air|manner)\b[^.!?;:]{0,50}\b(?:{re.escape(val)})\b|\b(?:{re.escape(val)})\b[^.!?;:]{0,60}\b(?:face|features|countenance|expression|look|air|manner)\b[^.!?;:]{0,60}(?:{nr}|\b(?:his|her|their)\b)',text):continue
                    cat='appearance' if attr not in {'clothing','expression','mannerism'} else attr
                    self._add_fact(con,pid,cat,attr,val,'scene',None,sid,ps,pe,self._evidence(text,m.start(),m.end()),min(.98,max(.35,bc)),'source_pattern_local')
    def _build_environment_facts(self,con,d,scenes,entity_profiles):
        for s in scenes:
            text=str(s['text'] or '')
            for eid,pid in list(entity_profiles.items()):
                typ=con.execute('SELECT entity_type FROM entities WHERE id=?',(eid,)).fetchone()
                if not typ or typ[0] not in {'location','environment'}:continue
                for cat,pat in self.ENV_PATTERNS.items():
                    for m in pat.finditer(text):self._add_fact(con,pid,cat,cat,self._clean(m.group(1)),'scene',None,int(s['id']),int(s['page_start']),int(s['page_end']),self._evidence(text,m.start(),m.end()),.65,'source_pattern')
    def _build_objects(self,con,d,scenes):
        ids={}
        for s in scenes:
        ...
        return ids
    def _build_scene_context(self,con,d,scenes,entity_profiles,object_ids):
        for s in scenes:
            sid=int(s['id']);mentions=con.execute('SELECT DISTINCT entity_id FROM entity_mentions WHERE document_id=? AND scene_id=?',(d,sid)).fetchall();chars=[];env=[]
            for r in mentions:
...
def main():
    p=__import__('argparse').ArgumentParser(description='Build source-grounded visual knowledge bible');p.add_argument('database');p.add_argument('document_id',type=int);a=p.parse_args();r=build_visual_knowledge_bible(a.database,a.document_id);print('=== VISUAL KNOWLEDGE BIBLE ===');[print(f'{k}: {v}') for k,v in r.items()]
